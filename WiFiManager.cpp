...
#ifdef MULTI_AP
  server->on(G(R_multiapsave),  std::bind(&WiFiManager::handleMultiAPSave, this));
...
void WiFiManager::HTTPSendJSON(String content){
  server->send(200, FPSTR(HTTP_HEAD_CT3), content);
}
...
#ifdef WM_DEBUG_LEVEL
  DEBUG_WM(F("Using MULTI_AP"));
#endif
  if(!SPIFFS.begin()){
#ifdef WM_DEBUG_LEVEL
    DEBUG_WM(F("SPIFFS Mount Failed"));
#endif
  }else{
    loadStoragedAPSet();
#ifdef WM_DEBUG_LEVEL
    DEBUG_WM(F("Current AP SET:"));
    DEBUG_WM(this->apSet.getAPSetAsJSON());
#endif
  }
}
...
#ifdef WM_DEBUG_LEVEL
    DEBUG_WM(F("WRONG MD5 CHECKSUM WHEN LOAD AP SET"));
    DEBUG_WM("MD5:");
    DEBUG_WM(md5);
    DEBUG_WM("json:");
    DEBUG_WM(json);
...
void WiFiManager::handleMultiAPSave() {

  #ifdef WM_DEBUG_LEVEL
  DEBUG_WM(DEBUG_VERBOSE,F("<- HTTP Multi AP Save "));
  #endif
  #ifdef WM_DEBUG_LEVEL
  DEBUG_WM(DEBUG_DEV,F("Method:"),server->method() == HTTP_GET  ? (String)FPSTR(S_GET) : (server->method() == HTTP_POST?(String)FPSTR(S_POST):(String)FPSTR(S_DELETE)));
  #endif
  handleRequest();

  HTTPSendJSON(doMultiAP());
...
String WiFiManager::doMultiAP(){
  ...
  if (server->method() == HTTP_GET){// send list wifi
    return this->apSet.getAPSetAsJSON();
  }
  else if (server->method() == HTTP_DELETE){

    if (server->hasArg(F("all"))){// delete all
      #ifdef WM_DEBUG_LEVEL
      ...
      #ifdef WM_DEBUG_LEVEL
      DEBUG_WM(F("delete ssid: "));
      DEBUG_WM(delSSID);
      #endif

      this->apSet.removeAP(delSSID);
      if (this->WiFi_SSID() == delSSID) {
        WiFi.disconnect(false, true);// disconnect wifi
      }
      this->storageAPSet();
    }
  ...
  else if (server->method() == HTTP_POST && server->hasArg(F("plain"))){// POST
    String jsonStr = server->arg(F("plain"));

    #ifdef WM_DEBUG_LEVEL
    DEBUG_WM(F("POST BODY:"));
    DEBUG_WM(jsonStr);
    #endif
    ...
    if (root.success()){
      ...
      String pass = root["pass"].as<String>();
      if (ssid!="") {
        this->apSet.setAP(ssid, pass);
        this->storageAPSet();
        statusMsg = "done";
...
String WiFiManager::getAPItemOut(){
    String page;
    int n = apSet.getAPAmount();

    if (n == 0) {
      page += FPSTR(S_aplistempty);
      page += F("<br/><br/>");
    }
    else {
      String HTTP_ITEM_STR = FPSTR(HTTP_AP_ITEM);

      for (int i = 0; i < n; i++) {
        APCredential c;
        if (!apSet.getAPAt(i, c)) continue;

        String item = HTTP_ITEM_STR;
        item.replace(FPSTR(T_e), c.getSSID());// ssid
        item.replace(FPSTR(T_p), c.getPass());// pass
