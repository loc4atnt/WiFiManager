...
"<hr><br/>", // MENU_SEP
"<form action='/aplist'    method='get'><button>Saved Access Point List</button></form><br/>\n"// MENU MULTI AP
};
...
const char HTTP_AP_LIST_ITEM[]     PROGMEM = "<div><h2>{v}</h2>{qi}</div>"; // {q} = HTTP_ITEM_QI, {r} = HTTP_ITEM_QP
...
const char HTTP_AP_ITEM[]          PROGMEM = "<div class='ele-aplist'><div class='body-aplist'><dt>{e}</dt><dd>{p}</dd></div><div class='rem-aplist'><i class='del-btn'>x</i></div></div>";
...
// multi_ap_list
".ele-aplist,.body-aplist,.rem-aplist{padding:0;margin:0;}"
".ele-aplist{display:flex;align-items:center;justify-content:space-between;}"
".rem-aplist .del-btn{color:red;cursor:default;}"
".ele-aplist:active{opacity:0.8;}"
"</style>";
...
const char HTTP_MULTI_AP_JS[]         PROGMEM = "<script>remBtnList=document.querySelectorAll('.rem-aplist');remBtnList.forEach((btn)=>{btn.onclick=(e)=>{parentSSID=e.target.closest('.ele-aplist');ssid=parentSSID.querySelector('.body-aplist dt').innerText;fetch(`/multiap?ssid=${ssid}`,{method:'DELETE'});parentSSID.remove();setTimeout(()=>{window.location.reload();},500);}});</script>";
...
const char S_DELETE[]               PROGMEM = "DELETE";
...
const char S_aplistempty[]           PROGMEM = "Saved access point list is empty!";
...
const char * const _menutokens[11] PROGMEM = {
    "wifi",
    "wifinoscan",
    "info",
    "param",
...
const char HTTP_HEAD_CT3[]        PROGMEM = "application/json";
